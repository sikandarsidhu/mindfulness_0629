...
public var returningHome = false; //used to tell wheather or not the animal has wandered too far from maxDistance and is now returning to startPos
private var distanceBetween : float; //distance animal is from startPos
var walkAnimation : String = "Walk"; //name of walk (or movement) animation | Most animals movement animation's are named "Walk", but for instance, if the duck was in water, you can change this name to "Swim", instead.
...
function NewDecision() {
	...
	if(idle == false) {
		...
		RandomIdle(false); //play randomIdle
	}
...
function RandomIdle(lookedAt : boolean) : boolean {
	//**RANDOM IDLE ANIMATION**
	//make sure there are idle animations to pick from
	if(IdleAnimation.length > 0 && lookedAt) {
...
public function StopMouse(){
	idle = true; //animal is now idle
	waitTime = Random.Range(minIdle, maxIdle); //get new waitTime
	RandomIdle(true); //play randomIdle
}
